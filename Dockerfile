...
RUN apt-get update \
 && apt-get -y upgrade \
 && apt-get -y install git unzip nano inotify-tools
...
RUN cd /etc/apache2/conf.d && sed -i -- 's/172.*.*.*:80/127.0.0.1:80/g' * && sed -i -- 's/172.*.*.*:8443/127.0.0.1:8443/g' * ; exit 0
RUN cd /etc/nginx/conf.d && sed -i -- 's/172.*.*.*:80 default;/80 default;/g' * && sed -i -- 's/172.*.*.*:8080/127.0.0.1:8080/g' * ; exit 0
...
EXPOSE 21 22 80 8083 3306 443 25 993 110 53 54
